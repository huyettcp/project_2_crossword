...
#Google with sampler
    ...
    # end
        ...
        front_arr.push front_story_text.split(" ")
        front_arr = front_arr.flatten.uniq
        front_arr.delete_if { |x| x.length <= 8 || exclude_words.include?(x) || exclude_names.include?(x) || exclude_girls_names.include?(x) }
        front_arr = front_arr.sample(150)
        count_front = front_arr.length
        front_arr_index = 0
